...
const ITEMS: Record<string, { name: string; chapter: string; story: string; img: string }> = {
...
export const Route = createFileRoute("/portfolio/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.item.name} — Dencyah Events` },
      { name: "description", content: loaderData?.item.story },
  ...
  return (
    <article className="bg-[var(--espresso)] text-[var(--cream)] min-h-dvh">
      <section className="relative h-[70vh] min-h-[480px] overflow-hidden">
        <img src={item.img} alt={item.name} className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative z-10 h-full flex flex-col justify-end px-6 md:px-14 pb-14 max-w-[1200px] mx-auto">
          ...
          <p className="mt-5 text-xs uppercase tracking-[0.3em] text-[var(--amber-gold)]">{item.chapter}</p>
          <h1 className="font-serif text-[clamp(2.5rem,6vw,5rem)] mt-3">{item.name}</h1>
        </div>
      </section>
      <section className="px-6 py-20 max-w-[760px] mx-auto">
        <p className="text-lg leading-relaxed text-[var(--champagne)]/85">{item.story}</p>
        <div className="mt-12">
